...
function ProduitCard({ prod, listeFavoris, setListeFavoris }) {
  ...
  return (
    <div key={prod.id} className="div-un-produit">
      ...
      <a href={'/produit?id=' + prod.id}> <img src={prod.image_url[0]} />
      </a>

      <div className="d-flex flex-row align-items-center justify-content-between">

        <div> <a href={'/produit?id=' + prod.id}><p>{prod.marque}</p> <p>{prod.nom}</p><p>{prod.prix}€</p>
        </a></div>
        {listeFavoris.filter(f => f == prod.id).length <= 0 ?
          <button id="btnAddHeart" onclick={() => { listeFavoris.push(prod.id) }}  >
