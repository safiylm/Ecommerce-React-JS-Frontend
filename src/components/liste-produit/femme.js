                ...
                liste_produits.filter(prod => prod.categorie[0] == categorie).filter(prod => prod.sexe == "Femme").map((number) =>
                    <div key={number.id} className="div-un-produit">
                        <a href={'/produit?id=' + number.id}>
                            <img src={number.image_url[0]} alt="image of produit" />
                            <div className="d-flex flex-row align-items-center justify-content-between">

                                <div><p>{number.marque}</p> <p>{number.nom}</p><p>{number.prix}€</p>
                                </div>
                                {panier.filter(p => p.id_produit == number.id).length <= 0 ?
                                    <button id="btnAddHeart" onclick={() => addInPanier(2)} >
                                    ...
                                    :
                                    <button id="btnRemoveHeart" onclick="removeInPanier()">
